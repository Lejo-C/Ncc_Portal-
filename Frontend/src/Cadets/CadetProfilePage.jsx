...
export default function CadetProfilePage() {
  ...
  const handleSendEmailVerification = async () => {
    try {
      await fetch("http://localhost:5000/api/users/send-email-verification", {
        method: "POST",
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-6 font-sans">
      <h1 className="text-2xl font-bold text-green-700 mb-6">👤 Cadet Profile</h1>
      ...
      <div className="bg-white p-6 rounded-xl shadow border border-gray-200 max-w-3xl mx-auto">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-8 gap-y-4 text-sm text-gray-700">
          ...
          <div><span className="font-semibold">Email:</span> {cadet?.email || "—"}</div>
          <div><span className="font-semibold">Phone:</span> {cadet?.phone || "—"}</div>
